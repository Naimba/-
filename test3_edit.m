...
for kks = [1 -1]% 控制扰源位置
    for ks = [1 -1]% 控制初始北传还是南传
        for ii = 1:5
            ...
            if ks == 1&&kks == 1
                [~,ind]=max(lat);
                m_text(lon(ind),lat(ind)+2,num2str(ii))
            elseif ks == -1&&kks == -1
                [~,ind]=min(lat);
                m_text(lon(ind),lat(ind)-2,num2str(ii))
            end
...
% toc
%% 图题等等
% title('纬向对称基流的定常波射线及传播距离','fontsize',12,'fontname','Yahei','Interpreter','tex','FontAngle','italic')
m_text(20,15,'5');m_text(-5,15,'1');m_text(-5,-15,'1');m_text(20,-15,'5');
% print(gcf,'F:\学习\毕业论文\复现李艳杰\纬向对称基流的定常波射线及传播距离','-dpng','-r400');
% close
%% beta_M
